 ...
 * @author Admin
 */
public class RandomTileSpawnEffectTest {
     static Game aGame;
     
        public RandomTileSpawnEffectTest() {
        }
         ...
         */
        @Test
        public void testRandomTileSpawnEffectNormalGame() {
            ...
            aGame.getBoard().getTileIJ(height - 3, 0).setEffect(new RandomTileSpawnEffect());
            ...
            assertTrue("Doit être d'effet randomTileSpawn", aGame.getBoard().getTileIJ(height - 3, 0).getEffect() instanceof RandomTileSpawnEffect);
            assertTrue(aGame.getCurrentPlayer().getId() != id_player);
            assertEquals(nb_tokens_before + 1, nb_tokens_after);
        }
     
        /**
         * Test de RandomTileSpawnEffect sur grille vide 
         * Vérification de l'état de la
         * tuile après application de l'effet 
         * Résultats attendus : la grille doit compter 2 pions
         */
        @Test
        public void testRandomTileSpawnEffectEmptyGame() {
     
            // Effet fixé sur une case 
            int height = aGame.getBoard().getHeight();
            aGame.getBoard().getTileIJ(height - 1, 0).setEffect(new RandomTileSpawnEffect());
            ...
            // Vérifications 
            // - le nombre de pions sur le plateau est de 2
            // - l'effet est bien appliqué sur la case 
            // - le tour de jeu a bien changé
            assertEquals(2, aGame.getBoard().getTotalTilesCount());
            assertTrue("Doit être d'effet randomTileSpawn", aGame.getBoard().getTileIJ(height - 1, 0).getEffect() instanceof RandomTileSpawnEffect);
            assertTrue(aGame.getCurrentPlayer().getId() != id_player);
     
        }
     
        /**
         * Test de RandomTileSpawnEffect sur grille pré-remplie 
         ...
         */
        @Test
        public void testRandomTileSpawnEffectFilledGame() {
            ...
            // Vérifications :
            // - le nombre de pions est incrementer de 2
            // - l'effet est bien appliqué sur la case 
            // - le tour de jeu a bien changé
            assertEquals(tile + 2, aGame.getBoard().getTotalTilesCount());
            assertTrue("Doit être d'effet randomTileSpawn", aGame.getBoard().getTileIJ(height - 3, 0).getEffect() instanceof RandomTileSpawnEffect);
            assertTrue(aGame.getCurrentPlayer().getId() != id_player);
